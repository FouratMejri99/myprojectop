...
const ProfileSettings = ({ setScreen }) => {
    ...
    try {
      ...
      if (agencyData.exists()) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            timestamp: Date.now(),
            data: agencyDataWithoutPassword,
          })
        );
  ...
  useEffect(() => {
    ...
    if (
      cachedAgency &&
      !isCacheExpired(JSON.parse(cachedAgency), CACHE_EXPIRATION_TIME)
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setErrors({
        ...
        agencyMatFiscal:
          !matFiscal.trim() && "Veuillez saisir votre Matricule Fiscal.",
        agencyPhone:
          !phone.trim() && "Veuillez saisir votre Numéro de Téléphone.",
        agencyEmail: !email.trim() && "Veuillez saisir votre E-mail.",
        agencyActivities:
          !activities.trim() && "Veuillez cocher Vos activités.",
        agencyPays: !pays.trim() && "Veuillez sélectionner Votre pays.",
        agencyAddress: !address.trim() && "Veuillez saisir votre Adresse.",
      });
    }
    if (
      name.trim() !== agency.name.trim() ||
      matFiscal.trim() !== agency.matFiscal.trim() ||
      phone.trim() !== agency.phone.trim() ||
      email.trim() !== agency.email.trim() ||
      activities.trim() !== agency.activities.trim() ||
      pays.trim() !== agency.pays.trim() ||
      address.trim() !== agency.address.trim() ||
      phone.trim() !== agency.phone.trim() ||
      email.trim() !== agency.email.trim() ||
      activities.trim() !== agency.activities.trim() ||
      pays.trim() !== agency.pays.trim() ||
      ...
      try {
        ...
        toast({
          title: "Erreur de mise à jour de votre profil.",
          variant: "destructive",
        });
      } finally {
        setIsSubmitting(false);
    ...
    if (photoURL instanceof File) {
      ...
      uploadTaskSnapshot.on(
        ...
        (error) => {
          ...
          toast({
            title: "Erreur de mise à jour de votre image de profil.",
            variant: "destructive",
          });
          setIsSubmitting(false);
        },
        async () => {
          ...
          let downloadURL = await getDownloadURL(
            uploadTaskSnapshot.snapshot.ref
          );
          await updateDoc(agencyRef, {
  ...
  const deleteImage = async () => {
    ...
    try {
      ...
      toast({
        title: "erreur de suppression de l'image de profil !",
        variant: "destructive",
      });
    } finally {
      setIsSubmitting(false);
        ...
        <div className="lg:flex gap-6">
          <Card className="rounded-xl xl:w-1/3 mb-8 lg:mb-0">
            <CardHeader className="flex items-center">
              <Avatar className="w-28 h-28 mb-6 cursor-pointer">
                ...
                <Label
                  htmlFor="img"
                  className="absolute h-full w-full flex flex-col items-center justify-center text-center text-white bg-black bg-opacity-50 opacity-0 hover:opacity-100 transition-opacity duration-200 cursor-pointer"
                >
                  <Camera />
            ...
            <CardContent className="mt-5 text-center">
              {photoURL && (
                <Button
                  ...
                  onClick={() => deleteImage()}
                >
                  Supprimer l'image de votre profil{" "}
                  {isSubmitting && (
          ...
          <Card className="rounded-xl flex-1">
            <CardHeader className="relative">
              <CardTitle className="absolute -top-3 left-4">
              ...
              <form onSubmit={(e) => handleSubmit(e)}>
                <div className="md:flex gap-4">
                  <div className="w-full relative mb-5">
                    <Label
                      htmlFor="name"
                      className="absolute -top-2 left-4 bg-card"
                    >
                      Nom
                    </Label>
                    <Input
                      ...
                      id="name"
                      placeholder="Nom"
                      className={`h-16 rounded-2xl hover:border-foreground focus:border-none transition-all`}
                      value={name}
                ...
                <div className="grid gap-1 mb-8">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="Plomberie"
                        checked={activities.includes("Plomberie")}
                        onCheckedChange={(check) => {
                          handleActivitiesChange("Plomberie", check);
                        }}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="Ménager"
                        checked={activities.includes("Ménager")}
                        onCheckedChange={(check) => {
                          handleActivitiesChange("Ménager", check);
                        }}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="Chaud/Froid"
                        checked={activities.includes("Chaud/Froid")}
                        onCheckedChange={(check) => {
                          handleActivitiesChange("Chaud/Froid", check);
                        }}
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="Electricité"
                        checked={activities.includes("Electricité")}
                        onCheckedChange={(check) => {
                          handleActivitiesChange("Electricité", check);
                        }}
                      />
                      <Label htmlFor="Electricité">Electricité</Label>
                    </div>
                  </div>
                  {errors.agencyActivities && (
                    <div className="text-destructive mt-2">
                      {errors.agencyActivities}
                    </div>
                  )}
                ...
                <div className="md:flex gap-4">
                  ...
                  <div className="w-full relative mb-5">
                    ...
                    <Textarea
                      placeholder="Adresse"
                      id="address"
                      className="rounded-2xl hover:border-foreground focus:border-none transition-all min-h-[63px]"
                      value={address}
                ...
                <div className="text-right">
                  <Button size="lg" className="text-md" disabled={isSubmitting}>
                    Enregistrer{" "}
                    {isSubmitting && (
                      <Loader2 className="ml-2 h-4 w-4 animate-spin" />
                    )}
