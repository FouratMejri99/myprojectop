...
const Interventions = ({ client }) => {
  ...
  const getAppointmentsByTnkerPro = async () => {
    ...
    if (
      cachedBookings &&
      JSON.parse(cachedBookings).data.length &&
      !isCacheExpired(
      ...
      setLastCheck(
        formatTimeSinceLastCheck(JSON.parse(cachedBookings).timestamp)
      );
    } else {
  ...
  useEffect(() => {
    if (client) getAppointmentsByTnkerPro();
    ...
    <Card className="rounded-xl">
      <CardHeader className="relative">
        <CardTitle className="absolute -top-5 left-3">
          <Button
            variant="link"
            className="sm:text-2xl text-primary-foreground dark:text-primary text-2xl font-semibold leading-none tracking-tight"
            onClick={() =>
              handleCacheClear(
                `${client.id}bookingpage`,
                setCurrentPage,
                setCachedData,
                cachedData
              )
        ...
        <CardDescription>
          Dernière vérification des données : {lastCheck}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Table>
                ...
                <TableRow key={booking.id}>
                  ...
                  <TableCell>
                    {new Date(booking.dateTime).getFullYear() +
                      "/" +
                      String(
                        new Date(booking.dateTime).getMonth() + 1
                      ).padStart(2, "0") +
                      "/" +
                      String(new Date(booking.dateTime).getDate()).padStart(
                        2,
                        "0"
                      ) +
                      " " +
                      String(new Date(booking.dateTime).getHours()).padStart(
                        2,
                        "0"
                      ) +
                      ":" +
                      String(new Date(booking.dateTime).getMinutes()).padStart(
                        2,
                        "0"
                      )}
                  </TableCell>
                  ...
                  <TableCell>
                    {new Date(booking.created_at).getFullYear() +
                      "/" +
                      String(
                        new Date(booking.created_at).getMonth() + 1
                      ).padStart(2, "0") +
                      "/" +
                      String(new Date(booking.created_at).getDate()).padStart(
                        2,
                        "0"
                      ) +
                      " " +
                      String(new Date(booking.created_at).getHours()).padStart(
                        2,
                        "0"
                      ) +
                      ":" +
                      String(
                        new Date(booking.created_at).getMinutes()
                      ).padStart(2, "0")}
                  </TableCell>
                </TableRow>
              ))
